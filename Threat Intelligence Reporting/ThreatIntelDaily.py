...
# Configure logging
LOG_FILE = "threatintel_daily.log"
...
def fetch_data(endpoint):
    ...
    try:
        ...
        logger.info(f"Successfully fetched data from endpoint: {endpoint}")
...
def generate_table(data):
    ...
    if df.empty:
        logger.warning("No data available to generate the table.")
        return None
...
def generate_bar_chart(df, column, output_image):
    ...
    plt.close()
    logger.info(f"Bar chart saved as {output_image}")
...
def generate_pie_chart(df, column, output_image):
    ...
    if not tag_counts:
        logger.warning("No tags available to generate pie chart.")
        return
    ...
    plt.title("Top 5 Most Common Tags", fontsize=16)
    plt.tight_layout()
    plt.savefig(output_image)
    plt.close()
    logger.info(f"Pie chart saved as {output_image}")
...
def generate_pdf_report(dataframe, bar_chart, pie_chart, output_pdf):
    ...
    logger.info(f"PDF report saved as {output_pdf}")
...
def send_email_with_attachment(subject, body, to_email, attachment_path):
    ...
    try:
        ...
        with smtplib.SMTP("smtp.gmail.com", 587) as server:
            server.starttls()
            logger.info("Logging into the SMTP server...")
            server.login(EMAIL_USER, EMAIL_PASSWORD)
            logger.info(f"Logged in successfully. Sending email to {to_email}...")
...
def main():
    logger.info("Starting threat intelligence data fetch and report generation.")
    pulses = fetch_data("pulses/subscribed")
    if not pulses or "results" not in pulses:
        logger.warning("No data available from the API.")
        return
    data = pulses["results"]
    df = generate_table(data)
    if df is None:
        logger.warning("Dataframe is empty, aborting report generation.")
        return
    ...
    logger.info("Threat intelligence report process completed.")

if __name__ == "__main__":
    scheduler = BlockingScheduler()
    scheduler.add_job(main, 'cron', hour=16, minute=0)  # 4:00 PM
    logger.info("Scheduler started. The report will be sent daily at 4:00 PM.")
    scheduler.start()